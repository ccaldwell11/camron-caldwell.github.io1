...
var range = function(x, y) { // almost did single-line but it got too complicated to comprehend. revisit
  if (x > y) {
    if (x - 1 > y) {
...
var reverseArr = function (array, output = []) {
  ...
  return reverseArr(array.slice(0, -1), output) //slice revisit
};
...
// nthFibo(7); // 13
// nthFibo(3); // 2
var nthFibo = function(n, current = 0, next = 1, count = 0) {
  if (n < 0) {
    return null;
  }
  ...
  return nthFibo(n, next, current + next, count + 1);
};
...
var compress = function(list) {
  ...
  const restOfList = list.slice(1);

  if (restOfList.length > 0 && currentElement === restOfList[0]) {
    return compress(restOfList);
  } else {
    return [currentElement].concat(compress(restOfList));
  }
};
...
var minimizeZeroes = function(array) {
  ...
  if (currentElement === 0 && restOfArray.length > 0 && restOfArray[0] === 0) {
    return minimizeZeroes(restOfArray);
  } else {
    return [currentElement].concat(minimizeZeroes(restOfArray));
  }
};
...
// alternateSign([-2,-7,8,3,-1,4]) // [2,-7,8,-3,1,-4]
var alternateSign = function(array, output = [], positive = 1) {
  if (array.length === 0) {
    return output;
  }
  ...
  return alternateSign(array.slice(1), output, !positive);
};

// 35. Given a string, return a string with digits converted to their word equivalent.
// Assume all numbers are single digits (less than 10).
// numToText("I have 5 dogs and 6 ponies"); // "I have five dogs and six ponies"
var numToText = function(str, current = str[0], stringWords = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']) {
  if (str.length === 0) {
    return '';
  }

  if (/[0-9]/.test(current)) {
    const digitWord = stringWords[Number(current)];
    return digitWord + numToText(str.slice(1));
  } else {
